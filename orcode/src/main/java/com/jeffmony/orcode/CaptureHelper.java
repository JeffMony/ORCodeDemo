...
public class CaptureHelper implements CaptureLifecycle,CaptureTouchEvent, CaptureManager, SurfaceHolder.Callback  {
     ...
     * 是否支持自动缩放（变焦），默认支持
     */
    private boolean mIsSupportAutoZoom = true;

    /**
     * 是否支持识别颜色反转色的码，黑白颜色反转，默认不支持
     */
    private boolean mIsSupportLuminanceInvert = false;

    /**
     * 是否支持连扫，默认不支持
     */
    private boolean mIsContinuousScan = false;
    /**
     * 连扫时，是否自动重置预览和解码器，默认自动重置
     */
    private boolean mIsAutoRestartPreviewAndDecode = true;
    /**
     * 是否播放音效
     */
     ...
     */
    private float mFramingRectRatio = 0.9f;
    /**
     * 识别区域垂直方向偏移量
     */
     ...
     */
    private float mTooDarkLux = AmbientLightManager.TOO_DARK_LUX;
    /**
     * 光线足够明亮，当光线亮度足够明亮，亮度高于此值时，隐藏手电筒按钮
     */
    private float mBrightEnoughLux = AmbientLightManager.BRIGHT_ENOUGH_LUX;
    /**
     * 扫码回调
     */
    ...
    /**
     * use {@link #CaptureHelper(androidx.fragment.app.Fragment, SurfaceView, ViewfinderView, View)}
     * @param fragment
     * @param surfaceView
    ...
    public void onCreate(){
        ...
        //设置闪光灯的太暗时和足够亮时的照度值
        mAmbientLightManager.setTooDarkLux(mTooDarkLux);
        mAmbientLightManager.setBrightEnoughLux(mBrightEnoughLux);
    ...
    public boolean onTouchEvent(MotionEvent event){
        if(mIsSupportZoom && mCameraManager.isOpen()){
    ...
    private void initCameraManager(){
        ...
        mCameraManager.setFullScreenScan(mIsFullScreenScan);
        mCameraManager.setFramingRectRatio(mFramingRectRatio);
    ...
    private void initCamera(SurfaceHolder surfaceHolder) {
        ...
        try {
            ...
            if (mCaptureHandler == null) {
                mCaptureHandler = new CaptureHandler(mActivity, mViewfinderView, mOnCaptureListener, mDecodeFormats, mDecodeHints, mCharacterSet, mCameraManager);
                mCaptureHandler.setSupportVerticalCode(mIsSupportVerticalCode);
                mCaptureHandler.setReturnBitmap(mIsReturnBitmap);
                mCaptureHandler.setSupportAutoZoom(mIsSupportAutoZoom);
                mCaptureHandler.setSupportLuminanceInvert(mIsSupportLuminanceInvert);
            }
        } catch (IOException ioe) {
            LogUtils.w(TAG, "initCamera failed", ioe);
     ...
     * 如果{@link #mIsContinuousScan}支持连扫，则默认重启扫码和解码器；当连扫逻辑太复杂时，
     * 请将{@link #autoRestartPreviewAndDecode(boolean)}设置为{@code false}，并手动调用{@link #restartPreviewAndDecode()}
     * @param result 扫码结果
     */
    public void onResult(Result result){
        final String text = result.getText();
        if(mIsContinuousScan){
            if(mOnCaptureCallback!=null){
                mOnCaptureCallback.onResultCallback(text);
            }
            if(mIsAutoRestartPreviewAndDecode){
    ...
    public CaptureHelper continuousScan(boolean isContinuousScan){
        this.mIsContinuousScan = isContinuousScan;
        return this;
    }
    ...
    public CaptureHelper autoRestartPreviewAndDecode(boolean isAutoRestartPreviewAndDecode){
        this.mIsAutoRestartPreviewAndDecode = isAutoRestartPreviewAndDecode;
        return this;
    }
    ...
    public CaptureHelper decodeFormats(Collection<BarcodeFormat> decodeFormats) {
        this.mDecodeFormats = decodeFormats;
        return this;
    }
    ...
    public CaptureHelper decodeHint(DecodeHintType key, Object value){
        if(mDecodeHints == null){
            mDecodeHints = new EnumMap<>(DecodeHintType.class);
        }
        mDecodeHints.put(key,value);
        return this;
    }
    ...
    public CaptureHelper supportVerticalCode(boolean supportVerticalCode) {
        this.mIsSupportVerticalCode = supportVerticalCode;
        if(mCaptureHandler!=null){
            mCaptureHandler.setSupportVerticalCode(mIsSupportVerticalCode);
        }
        return this;
    ...
    public CaptureHelper frontLightMode(FrontLightMode mode) {
        FrontLightMode.put(mActivity,mode);
        if(mIvTorch!=null && mode != FrontLightMode.AUTO){
            mIvTorch.setVisibility(View.INVISIBLE);
        }
        return this;
    }
    ...
    public CaptureHelper tooDarkLux(float tooDarkLux) {
        ...
        if(mAmbientLightManager != null){
            mAmbientLightManager.setTooDarkLux(tooDarkLux);
        }
        return this;
    }
    ...
    public CaptureHelper framingRectRatio(@FloatRange(from = 0.0f ,to = 1.0f) float framingRectRatio) {
        ...
        if(mCameraManager!=null){
            mCameraManager.setFramingRectRatio(framingRectRatio);
        }
        return this;
    }
    ...
    public CaptureHelper framingRectVerticalOffset(int framingRectVerticalOffset) {
        ...
        if(mCameraManager!=null){
            mCameraManager.setFramingRectVerticalOffset(framingRectVerticalOffset);
        }
        return this;
    }
    ...
    public CaptureHelper framingRectHorizontalOffset(int framingRectHorizontalOffset) {
        ...
        if(mCameraManager!=null){
            mCameraManager.setFramingRectHorizontalOffset(framingRectHorizontalOffset);
        }
        return this;
    }
    ...
    public CaptureHelper setOnCaptureCallback(OnCaptureCallback callback) {
        this.mOnCaptureCallback = callback;
        return this;
    }

    /**
     * {@link com.jeffmony.orcode.camera.CameraManager}
     * @return {@link #mCameraManager}
     */
    @Override
     ...
     * @return {@link #mAmbientLightManager}
     */
    @Override
